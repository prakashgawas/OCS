...
parser.add_argument('--learn_iter', type=int, default=400, help="Number of learning iterations")
...
parser.add_argument('--scenarios', type=int, default=1, help="Number of stochastic scenarios")
...
parser.add_argument('--new', type=int, default=0, help="Use new scenario")
...
def merge_files(folder: str,output_path:str,  output_file: str, files:str = "OCS", remain:str='schedule',  start_id=0, end_id=1000, scen = None, folder_name = "Evaluation"):
    dfs = []
    ...
    # Pivoted version
    agg_df = (
        df.groupby(['time', 'priority_is_p1', 'expert_action']) #iteration
        .size()
        .unstack(fill_value=0)
        .reset_index()
    )
    #print(agg_df.columns)
    agg_df["reject_per"] = agg_df[4]/(agg_df[0] + agg_df[1] + agg_df[2] + agg_df[3] + agg_df[4])
    agg_df["new"] =  args.new
    agg_df["scen"] = scen
    agg_df["Label"] = args.new + scen
    agg_df["iter"] = args.learn_iter
    
    agg_df.to_csv(f"{output_path}/Agg_actions_data_{args.new}_{scen}_{args.new + scen}_{args.learn_iter}.csv")
    ...
    for i in range(start_id, end_id + 1):
        ...
        filename = os.path.join(folder + folder_name + "/", name)
...
if __name__ == "__main__":
    ...
    base_path = f"../data/Data_Dagger_{args.lm}_{args.adapter}_{args.evolve_expert}/Instances_N25-75_s{args.s1}-{args.s2}_P4_I8_L20_{args.kmin}_4_{args.pw}_200_50_0_10/NN_iter{args.learn_iter}_runs{args.runs}_um1_new{args.new}_stoch{args.stoch}_sc{args.scenarios}_van1_gat0_tl{args.time_limit}_mg{args.mipgap}_nc5/"
    ...
    name = "Evaluation" #Evaluation_Validation #Evaluation
    folder_path = base_path   # or the folder where the files are located
    output_path = f"../data/Data_Dagger_{args.lm}_{args.adapter}_{args.evolve_expert}/{name}_N25-75_s{args.s1}-{args.s2}_P4_I8_L20_{args.kmin}_4_{args.pw}_200_50_0_10/"  # or the folder where the files are located
    os.makedirs(output_path, exist_ok=True)
    output_file_name = output_path + f"merged_sim_stats_NN_iter{args.learn_iter}_runs{args.runs}_um1_new{args.new}_stoch{args.stoch}_sc{args.scenarios}_van1_gat0_tl{args.time_limit}_mg{args.mipgap}_nc5.csv"
    merge_files(folder_path, output_path, output_file_name, files="Sim_stats", remain="", end_id  = args.learn_iter, scen = args.scenarios ,folder_name=name )
